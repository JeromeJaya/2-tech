...
const AdminBookingDetail = () => {
  ...
  const fetchBookingDetail = async (bookingId) => {
    try {
      ...
      if (data) {
        ...
        const balloonColorIds = safeParseJSON(data.balloon_colors, []);
        
        if (balloonColorIds.length > 0) {
          try {
            const response = await api.get('/balloon-colors', {
              params: { ids: balloonColorIds.join(',') }
            });
            
            if (response.data && response.data.success) {
              processedBalloonColors = response.data.data;
            }
  ...
  const updateBookingStatus = async (newStatus, newPaymentStatus) => {
    if (!booking) return;
