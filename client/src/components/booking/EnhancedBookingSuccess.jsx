...
export const EnhancedBookingSuccess = ({ bookingData, onNewBooking }) => {
  // Safe parsing function that handles both arrays and JSON strings
  const parseArraySafely = (field) => {
    if (!field) return [];
  ...
  const parseColorsSafely = (colors) => {
...
${
  selectedAddons.length > 0
    ? selectedAddons.map((addon) => `${addon.name || "Unknown"} x${addon.quantity || 1} - ₹${((addon.price || 0) * (addon.quantity || 1)).toLocaleString()}`).join("\n")
    : "None"
}
            ...
            {balloonColors.length > 0 && (
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {balloonColors.slice(0, 8).map((color, index) => (
                    <div
                      key={index}
