...
export const EnhancedBookingForm = ({ selectedPlan = "basic", onBookingComplete }) => {
  ...
  const checkSlotsAvailability = async (selectedDate) => {
    ...
    try {
      ...
      // Set all slots based on availability response
      timeSlots.forEach(slot => {
        availability[slot] = availableSlots.includes(slot);
      });
    } catch (error) {
