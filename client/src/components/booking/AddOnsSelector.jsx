...
import { getAddons } from "@/services/bookingService";

export const AddOnsSelector = ({ planType, selectedAddOns, onAddOnsChange }) => {
  ...
  const fetchAddOns = async () => {
    try {
      ...
      // Filter active add-ons based on plan type
  ...
  const getCustomName = (addOnId) => {
    const selected = selectedAddOns.find(item => item.id === addOnId);
    return selected ? selected.customName || '' : '';
  };

  const updateAddOn = (addOn, quantity, customName) => {
  ...
  const updateCustomName = (addOn, customName) => {
    const currentQuantity = getSelectedQuantity(addOn.id);
