...
export const ComprehensiveBookingForm = ({ onBookingComplete }: BookingFormProps) => {
  ...
  const uploadPhotosToSupabase = async (photos: File[]) => {
    const uploadedUrls = [];
        ...
        if (error) {
          ...
          if (error.message.includes('row-level security')) {
            toast({
              ...
              description: "Photos upload is temporarily disabled. Your booking will still be created.",
              variant: "destructive"
            });
          }
          continue;
  ...
  const checkAvailability = async () => {
    try {
      ...
      if (error) {
        ...
        // If there's an error, assume slot is available and let the user try
        return true;
      }

      // If data exists, slot is taken; if null, slot is available
      return !data;
    } catch (error) {
